function [ I_new_left,PW_left,I1_left,I2_left,I3_left] = left_Rechnung( I,p,X,Y,Part_left)
    ...
    for i_x = round(p(11,1)):round(p(3,1))
        ...
        %This marks the pixels of interest by marking the ROI with ones instead of inf
        I_logical_left(round(y_edge_up(2)):round(y_edge_down(2)),i_x)=1;
    end
    %Extract the pixels of interest from the 3 color channels by multiplying the mask with the imagedata
    ...
    p_I_left(3,:)=1;
    %Transform the coordinates with H matrix
    PW_left = Part_left*p_I_left;
